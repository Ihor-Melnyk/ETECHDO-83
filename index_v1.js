...
function setInitiatorOrg(isCreate = false) {
  ...
  var employeeData = EdocsApi.getEmployeeDataByEmployeeID(employeeId);
  if (employeeData) {
    setAttrValue("RegistrationType", employeeData.phone1);
    setAttrValue("InitiatorOrg", employeeData.phone3);
    setAttrValue("DateOfApplication", new Date());

    // обʼєкт із властивостями для відмінювання
    const objFullName = setObjFullNameByFullNameString(employeeData.fullName);
    ...
    const declinePositionName = declinePosition(employeeData.positionName, (caseType = "genitive"));

    setAttrValue("FirstNameLastNameDeclension", `${declineFirstName} ${declineLastName.toUpperCase()}`);
    setAttrValue("positionDeclension", declinePositionName);
  } else {
    setAttrValue("FirstNameLastNameDeclension", "");
    setAttrValue("positionDeclension", "");
...
// ===========================
//функціонал відмінювання посади у родовому/давальному відмінках
function declinePosition(positionName, caseType = "genitive") {
  if (positionName) {
    //масив похідних слів
